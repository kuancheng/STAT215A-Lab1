...
# Load the data.
setwd('~/Desktop/STAT215A/lab1/data/')
...
# First inspect the rough properties of the data. Let's work with the 'log'
# data first.
...
# This is a bit nicer using the data.table package
log.table <- data.table(log)
tables() 

# data.tables also let us compute functions on columns of our data
log.table[, .(avg.hum=mean(humidity)), by=nodeid]
...
# humidity vs temperature, hamatop as color.
ggplot(log[log.indices, ]) + geom_point(aes(x=humidity, y=humid_temp, 
					color=hamatop))

# add transparency
ggplot(log[log.indices, ]) + geom_point(aes(x=humidity, y=humid_temp, 
					color=hamatop), alpha=0.5)
...
# Do the individual datasets have repeated observations? How could we test 
# this?

# Exercise in class: for each nodeid / epoch combo in net, find the
# corresponding row in log if there is one, otherwise return NA.
############################################################################
...
# How many in net are not in log?
